...
built on Texas Water Development Board (TWDB) monitoring well **#5758402**.

**Live site:** https://kendall-county-water-table.trey-817.workers.dev

## What it shows

- An interactive chart of the water table from January 2021 to today (45k+ readings).
...
The TWDB feed (`/groundwater/well/5758402.json`) is ~8.5 MB and sends no CORS header, so
a browser can't fetch it directly. A Cloudflare Worker (`src/index.js`) proxies it
server-side at `/api/well`, downsamples it to a ~94 KB payload (last reading per day, per
month, plus raw last-30-days), adds CORS, and caches the result for one hour. The static
page (`public/index.html`, vanilla HTML + Chart.js) renders it.
