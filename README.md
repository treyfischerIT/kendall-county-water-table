...
- An animated, to-scale **cross-section illustration** — surface scene, soil, Glen Rose
  ...
  down to today's level while a ghost year counter ticks by (click to replay).
...
about twice a day (stale-while-revalidate, so visitors never wait on the refetch) to stay in
